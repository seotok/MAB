...
class MABRewriter:
    def __init__(self, bandit, samples_manager, initial_pending_list, rand=False):
        self.randomize_path = Utils.get_randomized_folder() 

        self.bandit = bandit
        self.samples_manager = samples_manager
        self.rand = rand
        self.initial_pending_list = initial_pending_list
    ...
    def run(self):
        ...
        if self.initial_pending_list is not None:
            self.samples_manager.load_samples_from_file('samples_initaial.pkl')
        else:
            self.samples_manager.get_initial_pending_list()
            self.samples_manager.save_samples_to_file('samples_initaial.pkl')
        ...
        total_pull_count = 0 + self.bandit.load_pullcount
        logger_rew.info('===========================================')
        process_count = 0
        count_skip = 0
        count_solve = 0
        ...
        if self.initial_pending_list is not None:
            self.samples_manager.load_samples_from_file(self.initial_pending_list)

        while True:
            sample = self.samples_manager.get_next_sample()
            if sample and os.path.exists(sample.current_exe_path):
            ...
            if total_pull_count % 500 == 0: 
                self.bandit.save_state(os.path.join("output", f'bandit_state_{total_pull_count}.pkl'), total_pull_count)
                self.samples_manager.save_samples_to_file(os.path.join("output", f'samples_{total_pull_count}.pkl'))
        ...
        while True:
            ...
            self.samples_manager.update_working_list()
            self.bandit.save_state(os.path.join("output", f'bandit_state_remaining.pkl'), total_pull_count + 1)
            self.samples_manager.save_samples_to_file(os.path.join("output", f'samples_{total_pull_count + 1}.pkl'))
            time.sleep(10)
