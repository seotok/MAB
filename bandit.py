...
class Bandit:
    ...
    def save_state(self, file_path, total_pull_count):
        state = {
            'list_arm': self.list_arm,
            'idx_to_ori_idx': self.idx_to_ori_idx,
            'c': self.c,
            '_as': self._as,
            '_bs': self._bs,
            'used_once_only_arm_idxs': self.used_once_only_arm_idxs,
            'random_arm_count': self.random_arm_count,
    ...
    def load_state(self, file_path):
        with open(file_path, 'rb') as file:
            ...
            self.list_arm = state['list_arm']
            self.idx_to_ori_idx = state['idx_to_ori_idx']
            self.c = state['c']
            self._as = state['_as']
            self._bs = state['_bs']
            self.used_once_only_arm_idxs = state['used_once_only_arm_idxs']
            self.random_arm_count = state['random_arm_count']
            self.load_pullcount = state['total_pull_count']

        print("list_arm:", self.list_arm)
        print("idx_to_ori_idx:", self.idx_to_ori_idx)
        print("c:", self.c)
        print("_as:", self._as)
        print("_bs:", self._bs)
        print("used_once_only_arm_idxs:", self.used_once_only_arm_idxs)
        print("random_arm_count:", self.random_arm_count)
        print("load_pullcount:", self.load_pullcount)
